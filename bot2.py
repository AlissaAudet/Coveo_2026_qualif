...
# ============================================================
# TWEAKABLE CONSTANTS (with descriptions)
# ============================================================

AGGRO_TICK = 120
# From this tick onward, the bot is allowed to play more aggressively (take riskier fights and value enemy territory more).

# ----------------------------
# Hub scoring (choosing each spore's "hub" target)
# ----------------------------
OWNED_BY_ME_PENALTY = 35
# Penalty applied when a hub candidate is already owned by us (prevents wasting time re-targeting our own tiles).

ENEMY_OWNED_BONUS = 180
# Bonus added to hub candidates that are enemy-owned after AGGRO_TICK (encourages pushing into enemy territory later).

NEUTRAL_OWNED_BONUS = 25
# Small bonus if a hub candidate is neutral-owned (only relevant when neutrals are allowed as hubs).

HUB_NUTRIENT_MIN = 1
# Minimum nutrients required on a tile to be considered as a valid hub candidate.

# ----------------------------
# Spawner hunting (chasing enemy spawners)
# ----------------------------
SPAWNER_HUNT_BONUS = 800
# Large incentive value used by older/alternate scoring systems to prioritize hunting enemy spawners (kept for tuning/extensions).

SPAWNER_HUNT_RADIUS = 30
# Only chase an enemy spawner if it is within this Manhattan distance from the spore.

CHASE_SPAWNER_BEFORE_TICK = 900
# Stop actively chasing enemy spawners after this tick (prevents wasting late-game turns on long chases).

# ----------------------------
# Ring behavior (expanding territory around the hub)
# ----------------------------
RING_MAX_EARLY = 4
# Maximum ring radius explored around the hub in the early game.

RING_MAX_MID = 7
# Maximum ring radius explored around the hub in the mid game.

RING_MAX_LATE = 10
# Maximum ring radius explored around the hub in the late game.

# ----------------------------
# Spawner creation (when and where to create spawners)
# ----------------------------
SPAWNER_COST_ALWAYS_OK_UNTIL = 7
# If nextSpawnerCost is at or below this value, always allow creating a spawner (cheap enough to be worth it).

SPAWNER_COST_OK_EARLY_UNTIL = 15
# In early game only, allow creating a spawner if nextSpawnerCost is at or below this value.

SPAWNER_EARLY_TICK = 220
# Defines what "early game" means for spawner creation logic (before this tick, we accept higher spawner costs).

NUTRIENT_FOR_SPAWNER = 45
# Create a spawner immediately if standing on a tile with at least this many nutrients (high-value production spot).

# ----------------------------
# Spore production (what biomass to spawn and how often)
# ----------------------------
CLAIMER_BIOMASS = 2
...
FIGHTER_EVERY_N = 6
# Produce a fighter when (number of spores % FIGHTER_EVERY_N == 0), assuming enough nutrients and tick >= 30.

WORKER_EVERY_N = 9
# Produce a worker when (number of spores % WORKER_EVERY_N == 0), assuming enough nutrients.

DESIRED_SPORES_BASE = 10
# Minimum baseline number of spores the bot tries to maintain.

DESIRED_SPORES_MAX_EXTRA = 100
# Maximum additional spores allowed from scaling with nutrients (caps runaway production).

DESIRED_SPORES_PER_NUTRIENTS = 15
# For each this many nutrients, increase desired spores by +1 (up to DESIRED_SPORES_MAX_EXTRA).

# ----------------------------
# Combat safety margins (how cautious we are entering occupied tiles)
# ----------------------------
ENEMY_MARGIN_BEFORE_AGGRO = 1
# Before AGGRO_TICK, require our biomass to exceed enemy biomass by this margin to enter/fight safely.
...
# ----------------------------
# Neutral avoidance behavior (pathfinding + "break neutrals only if stuck")
# ----------------------------
NEUTRAL_AS_WALL = False
# If True, neutral-owned tiles are treated like walls in pathfinding (we avoid stepping into neutrals by default).
...
# Hard cap on BFS node expansions per search to prevent pathfinding from eating too much time.
